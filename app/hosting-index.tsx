...
export function HostingIndex({onClose}:{onClose:()=>void}) {
  const [view,setView] = useState<"rates"|"packages"|"rentals"|"directory"|"method">("rates");
  ...
  return <section className="hosting-page" role="dialog" aria-modal="true" aria-label="Bitcoin miner hosting index">
    ...
    <nav className="hosting-tabs" aria-label="Hosting index sections">
      {([['rates','Hosting rates'],['packages','Buy + host'],['rentals','Hashrate rental'],['directory','Provider directory'],['method','Method & sources']] as const).map(([key,label])=><button key={key} className={view===key?"active":""} onClick={()=>setView(key)}>{label}</button>)}
    </nav>
